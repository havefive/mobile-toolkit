...
describe('ShellParserImpl', () => {
  ...
  describe('parseDoc', () => {
    it('should strip with default selector', (done: any) => {
      const mockScope = new MockWorkerScope();
      const parser = createMockedWorker(mockScope);
      const response = new MockResponse(template1);
      parser.parseDoc(response)
        .then((response: any) => response.text())
        .then((template: string) => {
          expect(normalize(template)).toBe(normalize(expectedResponse1_1));
          done();
        });
    });
    it('should strip with nested selector', (done: any) => {
      ...
      parser.parseDoc(response)
        .then((response: any) => response.text())
        .then((template: string) => {
          expect(normalize(template)).toBe(normalize(expectedResponse1_2));
          done();
        });
    });
    it('should strip with nested selector', (done: any) => {
      ...
      parser.parseDoc(response)
        .then((response: any) => response.text())
        .then((template: string) => {
          expect(normalize(template)).toBe(normalize(expectedResponse1_3));
          done();
        });
    });
